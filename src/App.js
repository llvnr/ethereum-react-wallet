...
function App() {
  ...
  const [balance, setBalance] = useState(0); // OBTENIR LA BALANCE
  const [amountSend, setAmountSend] = useState(); // ENVOYER DE LARGENT
  const [amountWithdraw, setAmountWithdraw] = useState(); // MONTANT A RETIRER
  ...
  async function transfer() {
    ...
    if(typeof window.ethereum !== "undefined") {
      const accounts = await window.ethereum.request({method: 'eth_requestAccounts'});
      const provider = new ethers.providers.Web3Provider(window.ethereum);
      const signer = provider.getSigner(); // Pour modifier des données dans la blockchain.

      try {
        const tx = {
          from: accounts[0],
          to: WalletAddress,
          value: ethers.utils.parseEther(amountSend) // Convertion en ethers
        }
